...
<template>
  <PageShell
    title="活动邀请"
    description="向指定学员发送活动邀请（可选）；学员也可在企业详情页自行报名"
    :loading="loading"
    :error="loadError"
    @retry="fetchData"
  >
    ...
    <el-alert
      ...
      :closable="false"
      show-icon
      title="定向邀请适用于邀请特定学员；公开活动仍可在企业详情页由学员主动报名。"
      class="tip"
    />
    ...
    <el-empty v-if="!loading && invitations.length === 0" class="page-empty" :image-size="80" description="暂无活动邀请" />
  </PageShell>
    ...
    <el-form label-width="88px">
      <el-form-item label="活动" required>
        <el-select v-model="form.activityId" placeholder="选择活动" style="width: 100%">
          <el-option v-for="item in invitableActivities" :key="item.id" :label="item.title" :value="item.id" />
        </el-select>
      </el-form-item>
